...
export default function DepartmentDetailError({ error, reset }: ErrorProps) {
    ...
    <div className="max-w-2xl mx-auto py-12 px-4">
      <div className="bg-red-50 border border-red-200 rounded-lg p-8 text-center">
        <h2 className="text-xl font-semibold text-red-900 mb-2">
          {isNotFound ? 'Department not found' : 'Failed to load department'}
        </h2>
        <p className="text-red-800 mb-6 text-sm">
          {isNotFound
            ? 'This department may have been deleted.'
            : error.message || 'An error occurred while loading the department. Please try again.'}
        </p>
