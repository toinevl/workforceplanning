---
project: Workforce Planning
owner: toine
updated: 2026-06-28
tags: [wishlist]
---

# Wishlist

- [x] (A) Complete enterprise-departments feature work and merge +feature @me #0
- [x] (A) Hardening/validation for department detail page +feature +ui @me #0
- [ ] (B) Revisit load/error/empty states across department surfaces +feature @me #1
- [x] (B) Capture remaining phase learnings and convert to actionable follow-ups +docs @me #2
- [x] (C) Add small quantitative preview for reorg impact before apply +feature @me #3
- [ ] (C) Audit and tighten Azure Table Storage query paths +perf +security @me #4
- [~] (A) Validate Azure remote app failure and rebuild Azure services using learned best practices +infra +azure @me #5
- [ ] (B) Add Application Insights / observability to App Service +infra @me #6
- [ ] (B) Add staging slot and promote pipeline for deployments +infra @me #7
- [x] (B) Remove long-lived SAS / secret leakage risk from deploy pipeline +security +infra @me #8
- [ ] (C) Migrate to App Service Standard tier for production runtime +infra @me #9
- [x] (D) Review and incorporate Azure best-practices doc into architecture decisions +docs +infra @me #10
- [ ] (D) Add mapper exhaustiveness type check (entity→domain boundary) +code-quality @me #11
- [ ] (D) Add Content-Security-Policy header (restrict style-src) +security @me #12
- [ ] (D) Document seed/scripting standardization +docs @me #13
- [ ] (D) Create Architecture Decision Records (ADR) +docs @me #14
